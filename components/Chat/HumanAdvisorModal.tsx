...
export function HumanAdvisorModal({ isOpen, onClose, categoryKey, subcategoryTitle, selectedAdvisor }: HumanAdvisorModalProps) {
  ...
  const fetchAdvisors = async () => {
    setIsLoading(true)
    try {
      const getAvailableAdvisors = httpsCallable(functions, 'getAvailableAdvisors')
  ...
  const handlePayment = async (advisorId: string) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>{selectedAdvisor ? 'Payment Options' : 'Connect with Human Advisor'}</DialogTitle>
          <DialogDescription>
            {selectedAdvisor 
              ? `Choose payment method to start chat with ${selectedAdvisor.name}`
              : `Get personalized advice from our expert ${subcategoryTitle || 'advisors'}.`}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <div className="flex items-center justify-between bg-blue-50 p-4 rounded-2xl border border-blue-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-[10px] font-bold text-blue-600 uppercase tracking-wider">Wallet Balance</div>
                <div className="text-lg font-black text-blue-900">₹{(walletBalance / 100).toFixed(2)}</div>
              </div>
            </div>
          ...
          {selectedAdvisor ? (
            <div className="space-y-4">
              <div className="p-4 border rounded-2xl bg-slate-50 flex items-center gap-4">
                ...
                <div>
                  <div className="font-black text-gray-900">{selectedAdvisor.name}</div>
                  <div className="text-xs font-medium text-gray-500">
                    {Array.isArray(selectedAdvisor.specialty) ? selectedAdvisor.specialty.join(', ') : selectedAdvisor.specialty}
                  </div>
                  <div className="flex items-center gap-1.5 mt-1">
                    <Star className="w-3.5 h-3.5 text-yellow-400 fill-yellow-400" />
                    <span className="text-xs font-bold text-gray-700">{selectedAdvisor.rating || '4.8'}</span>
                    <span className="text-[10px] text-gray-400 font-medium ml-1">{selectedAdvisor.experience || '5+ years'} exp</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3">
                <Button 
                  onClick={() => handlePayment(selectedAdvisor.id)}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span>{walletBalance >= price * 100 ? 'Pay with Wallet' : 'Pay with Razorpay'}</span>
                  </div>
            ...
            <>
              <div className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1">Available Experts</div>

              {isLoading ? (
                <div className="flex flex-col items-center py-12">
                  <Loader2 className="w-10 h-10 animate-spin text-blue-600 mb-3" />
                  <p className="text-sm font-bold text-gray-400">Finding the best experts for you...</p>
                </div>
              ) : advisors.length > 0 ? (
                  ...
                  {advisors.map((advisor) => (
                    <div key={advisor.id} className="flex items-center justify-between p-4 border border-gray-100 rounded-2xl hover:border-blue-200 hover:bg-blue-50/30 transition-all group">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="font-bold text-gray-900 group-hover:text-blue-700 transition-colors">{advisor.name}</div>
                          <div className="text-[11px] font-medium text-gray-500 line-clamp-1">
                            {Array.isArray(advisor.specialty) ? advisor.specialty.join(', ') : advisor.specialty}
                          </div>
                          <div className="flex items-center gap-1.5 mt-1">
                            <Star className="w-3 h-3 text-yellow-400 fill-yellow-400" />
                            <span className="text-[10px] font-bold text-gray-700">{advisor.rating || '4.9'}</span>
                            <span className="text-[10px] text-gray-400 font-medium ml-1">{advisor.experience || '4+ years'} exp</span>
                          </div>
                        </div>
                      </div>
                      <Button 
                        size="sm" 
                        onClick={() => handlePayment(advisor.id)}
                        disabled={isProcessing}
                        className="bg-blue-600 hover:bg-blue-700 rounded-xl font-bold px-4"
                      >
                        {isProcessing ? <Loader2 className="w-3 h-3 animate-spin" /> : `₹${price}`}
                      </Button>
                    </div>
                  ))}
                </div>
                ...
                <div className="text-center py-12 border-2 border-dashed border-gray-100 rounded-3xl">
                  ...
                  <p className="text-sm font-bold text-gray-500">No advisors available right now.</p>

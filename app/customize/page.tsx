...
const DUMMY_MEDICAL_REPORTS: UploadedFile[] = [
    { id: "1", name: "Blood_Test_Report_Feb2026.pdf", type: "Lab Report", size: "2.4 MB", status: "analyzed", date: "Feb 18, 2026" },
    { id: "2", name: "Chest_Xray_Jan2026.dcm", type: "X-Ray", size: "8.1 MB", status: "analyzed", date: "Jan 22, 2026" },
    { id: "3", name: "MRI_Brain_Scan.dcm", type: "MRI", size: "45.2 MB", status: "processing", date: "Feb 20, 2026" },
];

const DUMMY_GENETIC_REPORTS: UploadedFile[] = [
    { id: "1", name: "Full_Genome_Sequence.vcf", type: "DNA Analysis", size: "120 MB", status: "analyzed", date: "Dec 10, 2025" },
    { id: "2", name: "Oncology_Panel_Results.pdf", type: "Oncology", size: "3.8 MB", status: "processing", date: "Feb 15, 2026" },
];

const GENETIC_CATEGORIES = ["DNA Analysis", "Oncology", "Carrier Screening", "Pharmacogenomics"];
...
function StatusBadge({ status }: { status: UploadedFile["status"] }) {
    const config: Record<UploadedFile["status"], { style: string; icon: React.ReactNode; label: string }> = {
        analyzed: { style: "bg-primary/15 text-primary", icon: <CheckCircle2 size={12} />, label: "Analyzed" },
        processing: { style: "bg-amber-100 text-amber-700", icon: <Clock size={12} className="animate-spin" />, label: "Processing" },
        uploading: { style: "bg-blue-100 text-blue-700", icon: <Loader2 size={12} className="animate-spin" />, label: "Uploading" },
        uploaded: { style: "bg-emerald-100 text-emerald-700", icon: <CheckCircle2 size={12} />, label: "Uploaded" },
    };
...
function FileCard({ file, onRemove }: { file: UploadedFile; onRemove: (id: string) => void }) {
    ...
    return (
        <div className={`flex items-center gap-3 bg-white border rounded-2xl px-4 py-3 group transition-all ${isUploading ? "border-blue-200 bg-blue-50/30" : "border-gray-200 hover:border-primary/30 hover:shadow-sm"}`}>
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${isUploading ? "bg-blue-100" : "bg-primary/10"}`}>
                {isUploading ? <Loader2 size={18} className="text-blue-600 animate-spin" /> : <FileText size={18} className="text-primary" />}
            </div>
            <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-gray-900 truncate">{file.name}</p>
                <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-[10px] text-gray-400 font-medium">{file.type}</span>
                    <span className="text-[10px] text-gray-300">•</span>
                    <span className="text-[10px] text-gray-400 font-medium">{file.size}</span>
                    <span className="text-[10px] text-gray-300">•</span>
                    <span className="text-[10px] text-gray-400 font-medium">{file.date}</span>
                </div>
...
function UploadZone({ onUpload, acceptLabel, inputRef, accept }: {
    onUpload: () => void;
    acceptLabel: string;
...
export default function CustomizePage() {
    ...
    const [medicalFiles, setMedicalFiles] = useState<UploadedFile[]>(DUMMY_MEDICAL_REPORTS);
    const medicalInputRef = useRef<HTMLInputElement>(null);

    // Genetic Reports state
    const [geneticFiles, setGeneticFiles] = useState<UploadedFile[]>(DUMMY_GENETIC_REPORTS);
    ...
    const [trackers, setTrackers] = useState<HealthTracker[]>([
        {
            ...
            connecting: false,
            metrics: [
                { label: "Steps", value: "8,432", icon: <Footprints size={14} className="text-primary" /> },
                { label: "Heart Rate", value: "72 bpm", icon: <Heart size={14} className="text-red-500" /> },
                { label: "Sleep", value: "7h 22m", icon: <Moon size={14} className="text-indigo-500" /> },
            ],
        },
        {
            id: "samsung",
            ...
            connecting: false,
            metrics: [
                { label: "Steps", value: "6,120", icon: <Footprints size={14} className="text-primary" /> },
                { label: "Calories", value: "1,840", icon: <Flame size={14} className="text-orange-500" /> },
                { label: "Sleep", value: "6h 45m", icon: <Moon size={14} className="text-indigo-500" /> },
            ],
        },
        {
            id: "google",
            ...
            connecting: false,
            metrics: [
                { label: "Steps", value: "9,870", icon: <Footprints size={14} className="text-primary" /> },
                { label: "Heart Rate", value: "68 bpm", icon: <Heart size={14} className="text-red-500" /> },
                { label: "Calories", value: "2,100", icon: <Flame size={14} className="text-orange-500" /> },
            ],
        },
    ]);
    ...
    const simulateFileUpload = useCallback((
        ...
        newFiles.forEach((newFile) => {
            setTimeout(() => {
                setFiles((prev) =>
                    prev.map((f) => (f.id === newFile.id ? { ...f, status: "uploaded" as const } : f))
                );
            }, 1500);
            ...
            setTimeout(() => {
                setFiles((prev) =>
                    prev.map((f) => (f.id === newFile.id ? { ...f, status: "analyzed" as const } : f))
                );
            }, 5500);
        });
    }, []);
    ...
    const handleMedicalDrop = () => {
        ...
        setTimeout(() => setMedicalFiles((prev) => prev.map((f) => f.id === fakeFile.id ? { ...f, status: "uploaded" } : f)), 1500);
        setTimeout(() => setMedicalFiles((prev) => prev.map((f) => f.id === fakeFile.id ? { ...f, status: "processing" } : f)), 3000);
        setTimeout(() => setMedicalFiles((prev) => prev.map((f) => f.id === fakeFile.id ? { ...f, status: "analyzed" } : f)), 5500);
    };
    ...
    const handleGeneticDrop = () => {
        ...
        setTimeout(() => setGeneticFiles((prev) => prev.map((f) => f.id === fakeFile.id ? { ...f, status: "uploaded" } : f)), 1500);
        setTimeout(() => setGeneticFiles((prev) => prev.map((f) => f.id === fakeFile.id ? { ...f, status: "processing" } : f)), 3000);
        setTimeout(() => setGeneticFiles((prev) => prev.map((f) => f.id === fakeFile.id ? { ...f, status: "analyzed" } : f)), 5500);
    };
    ...
    const toggleTracker = (id: string) => {
        ...
        if (tracker.connected) {
            // Instant disconnect
            setTrackers((prev) =>
                prev.map((t) => (t.id === id ? { ...t, connected: false, connecting: false } : t))
            );
            return;
        }

        // Start connecting → show loading for 2s → connected
        setTrackers((prev) =>
            prev.map((t) => (t.id === id ? { ...t, connecting: true } : t))
        );

        setTimeout(() => {
            setTrackers((prev) =>
                prev.map((t) => (t.id === id ? { ...t, connected: true, connecting: false } : t))
            );
        }, 2000);
    };
        ...
        <div className="flex flex-col h-full pb-24 overflow-y-auto">
            ...
            <div className="px-5 space-y-5">
                    ...
                    {medicalFiles.length > 0 && (
                        <div className="mt-4 space-y-2">
                            {medicalFiles.map((file) => (
                                <FileCard key={file.id} file={file} onRemove={removeMedicalFile} />
                            ))}
                        </div>
                    )}
                </SectionCard>
                    ...
                    {geneticFiles.length > 0 && (
                        <div className="mt-4 space-y-2">
                            {geneticFiles.map((file) => (
                                <FileCard key={file.id} file={file} onRemove={removeGeneticFile} />
                            ))}
                        </div>
                    )}
                </SectionCard>
                ...
                >
                    <div className="space-y-3">
                        {trackers.map((tracker) => (
                            <div key={tracker.id} className={`border rounded-2xl overflow-hidden transition-all ${tracker.connecting ? "border-primary/40 bg-primary/5" : tracker.connected ? "border-primary/30" : "border-gray-200 hover:border-primary/30"}`}>
                                {/* Tracker Header */}
                      ...
                      ${tracker.connecting
                                    ...
                                    >
                                        {tracker.connecting && <Loader2 size={12} className="animate-spin" />}
                                        {tracker.connecting
                                            ? "Syncing..."
                                            : tracker.connected
                                                ? "Disconnect"
                                ...
                                {tracker.connecting && (
                                    <div className="px-4 pb-3">
                                        <div className="w-full bg-primary/10 rounded-full h-1.5 overflow-hidden">
                                            <div className="bg-primary h-full rounded-full animate-[loading_2s_ease-in-out_forwards]" style={{
                                                animation: "loading 2s ease-in-out forwards",
                                            }} />
                                        </div>
                                        <p className="text-[10px] text-primary/70 font-medium mt-1.5 text-center">
                                ...
                                {tracker.connected && !tracker.connecting && tracker.metrics && (
                                        ...
                                        <div className="grid grid-cols-3 gap-2">
                                            {tracker.metrics.map((m) => (
                                                <div key={m.label} className="bg-gray-50 rounded-xl px-3 py-2.5 text-center">
                                                    ...
                                                    <p className="text-[9px] text-gray-400 font-bold uppercase tracking-wider">{m.label}</p>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                ...
                >
                    ...
                    {/* Quick Questions */}
                    <div>
                        <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2.5">Quick Questions</p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            {QUICK_QUESTIONS.map((q, i) => (
                                ...
                                >
                                    <MessageCircle size={14} className="text-gray-400 group-hover:text-primary shrink-0 transition-colors" />
                                    <span className="text-xs font-medium text-gray-600 group-hover:text-primary transition-colors">{q}</span>
                                    <ChevronRight size={12} className="ml-auto text-gray-300 group-hover:text-primary shrink-0 transition-colors" />
                                </button>
                            ))}
                        </div>
                    </div>
            ...
            {/* CSS keyframe for loading bar animation */}
            <style jsx>{`
        @keyframes loading {
          0% { width: 0%; }
          50% { width: 60%; }
